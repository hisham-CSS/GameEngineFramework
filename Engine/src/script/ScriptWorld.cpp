...
namespace MyCoreEngine {
    ...
    bool ScriptWorld::defaultResolve_(const std::string& path, std::string& out) const {
        ...
        std::error_code ec;
        fs::path full;
        // The script path comes straight out of an untrusted scene file and,
        // via the Lua loader, an escaping path could once have executed a file
        // outside the project. PathIsContained (see PathSandbox.h) rejects
        // absolute/root/".." paths before any filesystem read. Containment
        // failure returns false like any unreadable path; Build reports it to
        // the console as "could not read script '<path>'".
