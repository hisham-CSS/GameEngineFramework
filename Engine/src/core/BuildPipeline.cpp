...
//
//   * HOW "WILL THIS PROFILE INSTALL A PLAYER" IS ANSWERED WITH NOTHING BUILT --
//     see `ProfileHasInstallRule`. Both players have an install rule now and each
//     is restricted to the configurations its profile selects, so the question
//     stopped being "which target" and became "which CONFIGURATION". The answer
//     is read out of the config guard CMake writes around each rule in the
//     generated install script, which is the same file the install will run.
//
...
namespace MyCoreEngine {
    ...
    namespace {
        ...
        // -----------------------------------------------------------------
        // Will this profile's install rule put a player in the bundle?
        // -----------------------------------------------------------------
        // THE QUESTION CHANGED SHAPE. It used to be "is PlayerDebug installed at
        // all", because `install(TARGETS PlayerShipping ...)` was the tree's only
        // player rule and a Debug or Development bundle came out with no
        // executable in it. Player/CMakeLists.txt now installs BOTH players and
        // restricts each to the configurations its profile selects -- Release for
        // PlayerShipping, Debug and RelWithDebInfo for PlayerDebug -- so that a
        // Shipping bundle does not also carry a console-window player and leave a
        // user guessing which file is the game. What can go wrong is therefore no
        // longer a missing target but a MISSING CONFIGURATION: a build tree
        // generated before those rules existed, or a configuration no profile
        // names.
        //
        // SO THE GENERATED INSTALL SCRIPT IS ASKED, AND ASKED PER CONFIGURATION.
        // CMake writes one cmake_install.cmake per directory that installs
        // anything, names each file by full path, and -- this is what makes the
        // question answerable in advance -- wraps a CONFIGURATIONS-restricted rule
        // in its own guard:
        //
        //   if(CMAKE_INSTALL_CONFIG_NAME MATCHES "^([Rr][Ee][Ll][Ee][Aa][Ss][Ee])$")
        //     file(INSTALL ... FILES ".../Player.exe")
        //   endif()
        //
        // Multi-config generators spell a multi-configuration rule as one guard
        // per config joined by `elseif`, which is why the walk below tracks
        // `elseif` and `else` rather than only `if`/`endif`. Nothing here is
        // executed and nothing is spawned: preflight reads the file the install
        // would run.
        //
        ...
        //
        // AND IT MUST BE A `file(INSTALL ...)` LINE, which is not pedantry -- it is
        // the difference between this check working and this check being unable to
        // fail at all. vcpkg's applocal hook (X_VCPKG_APPLOCAL_DEPS_INSTALL, root
        // CMakeLists.txt:6) appends an UNGUARDED install(CODE) for every installed
        // target, and that block names the executable twice:
        //
        //   message("-- Installing app dependencies for PlayerDebug...")
        //   execute_process(COMMAND ...
        //       -targetBinary "${CMAKE_INSTALL_PREFIX}/./PlayerDebug.exe" ...)
        //
        // Counted, those make every player look installed in every configuration,
        // because they sit outside the config guard the actual rule is inside.
        // Measured on this tree, not reasoned about: an earlier version of this
        // function answered Present for PlayerDebug in Release. `file(INSTALL` is
        // the line that copies a file and the only thing CMake writes for an
        // installed target, so that is what is counted.
        //
        // WHAT IT REFUSES TO GUESS ABOUT COMES BACK AS Unknown, NOT Absent, and
        // that direction is deliberate: Absent is a refusal, so anything this
        // function is unsure of must not produce one. Three cases reach it -- no
        // install script readable at all (a tree generated by something else), a
        // config guard whose encoding is not the one below, and a script set with
        // no `file(INSTALL` line anywhere in it, which means the generated shape
        // is not the one being read. VALIDATE checks the assembled bundle for the
        // executable either way, so an Unknown costs a note in the panel rather
        // than a bundle nobody checked.
        enum class InstallRuleAnswer { Present, Absent, Unknown };

        // How CMake spells a configuration name inside a generated config guard:
        // every letter becomes a two-character class, uppercase first, and
        // anything else passes through. "Release" -> "[Rr][Ee][Ll][Ee][Aa][Ss][Ee]".
        //
        // CONSTRUCTED, NOT PARSED. Decoding the bracket expression out of the
        // guard would have to be right about CMake's encoding to produce ANY
        // answer; building the pattern and looking for it only has to be right to
        // produce a POSITIVE one, and a guard spelled some other way falls through
        // to Unknown instead of to a refusal.
        std::string EncodeConfigForInstallScript(const std::string& config) {
            std::string out;
            out.reserve(config.size() * 4);
            for (char c : config) {
                if (c >= 'a' && c <= 'z') {
                    out += '[';
                    ...
                    out += c; // digits and separators are emitted literally
                }
            }
            return out;
        }

        std::string TrimLeadingSpace(const std::string& s) {
            std::size_t i = 0;
        ...
        // A CONFIG TEST, not merely a line that says CMAKE_INSTALL_CONFIG_NAME.
        // BOTH HALVES ARE REQUIRED and the second one is why: every generated
        // install script opens with
        //
        //   if(NOT DEFINED CMAKE_INSTALL_CONFIG_NAME)
        //
        // which names the variable and tests nothing about it. Treated as a guard
        // it would put the whole preamble under a configuration nothing matches --
        // harmless today because no rule is emitted inside it, and exactly the kind
        // of accident that stops being harmless when CMake rearranges its preamble.
        // `MATCHES` is what CreateConfigTest always writes and what `NOT DEFINED`
        // never does.
        bool IsConfigGuardLine(const std::string& t) {
            return t.find("CMAKE_INSTALL_CONFIG_NAME") != std::string::npos &&
                   t.find("MATCHES") != std::string::npos;
        }
        ...
        InstallRuleAnswer ProfileHasInstallRule(const fs::path& binaryDir,
            ...
            // The guard we are looking for, and the ones CMake could write
            // instead. A guard holding none of these is one this function does not
            // understand, which is Unknown rather than "not our configuration".
            const std::string want = EncodeConfigForInstallScript(config);
            const std::string known[] = {
                EncodeConfigForInstallScript("Debug"),
                EncodeConfigForInstallScript("Release"),
                EncodeConfigForInstallScript("RelWithDebInfo"),
                EncodeConfigForInstallScript("MinSizeRel"),
            };
            ...
            bool sawUnreadableGuard = false;
            // Whether these scripts are shaped the way this function reads them at
            // all. Tracked across every script rather than per script, because a
            // directory that installs nothing legitimately has no such line.
            bool sawAnyFileInstall = false;
            ...
            for (const fs::path& s : scripts) {
                ...
                // `depth` counts open if() blocks; `guardDepth` is the depth of
                // the config guard currently in force, or 0 for none. Nested
                // if()s inside a guard (the strip block CMake emits after every
                // installed executable is one) move `depth` and leave the guard
                // alone, which is the whole reason this counts rather than
                // clearing on the first endif().
                int depth = 0;
                int guardDepth = 0;
                bool guardMatches = false;
                bool guardRecognised = true;

                auto readGuard = [&](const std::string& t) {
                    guardMatches = t.find(want) != std::string::npos;
                    guardRecognised = false;
                    for (const std::string& k : known) {
                        if (t.find(k) != std::string::npos) {
                            guardRecognised = true;
                            break;
                        }
                    }
                };
                ...
                while (std::getline(in, line)) {
                    const std::string t = TrimLeadingSpace(line);
                    const bool isEndIf = StartsWith(t, "endif(");
                    const bool isElseIf = StartsWith(t, "elseif(");
                    const bool isElse = StartsWith(t, "else(");
                    const bool isIf = !isElseIf && StartsWith(t, "if(");

                    // TESTED BEFORE `depth` MOVES, so the guard consulted is the
                    // one ENCLOSING this line rather than any block the line
                    // itself opens.
                    const bool isFileInstall =
                        t.find("file(INSTALL") != std::string::npos;
                    if (isFileInstall) sawAnyFileInstall = true;

                    if (isFileInstall &&
                        (t.find(anchoredSlash) != std::string::npos ||
                         t.find(anchoredBack) != std::string::npos)) {
                        if (guardDepth == 0) {
                            // No configuration restriction: installed always.
                            return InstallRuleAnswer::Present;
                        }
                        if (guardMatches) return InstallRuleAnswer::Present;
                        if (!guardRecognised) sawUnreadableGuard = true;
                    }

                    if (isIf) {
                        if (guardDepth == 0 && IsConfigGuardLine(t)) {
                            guardDepth = depth + 1;
                            readGuard(t);
                        }
                        ++depth;
                        ...
                        // A multi-config generator writes one branch per
                        // configuration, so the guard's answer changes here rather
                        // than at a new if().
                        if (guardDepth != 0 && depth == guardDepth) {
                            if (IsConfigGuardLine(t)) {
                                readGuard(t);
                            }
                            else {
                                guardMatches = false;
            ...
            if (sawUnreadableGuard || !sawAnyFileInstall) {
                return InstallRuleAnswer::Unknown;
            }
        ...
        BuildPreflight RunPreflight(const BuildSettings& settings,
            ...
            //
            // AND THE CONFIGURATION-RESTRICTED INSTALL RULES DO NOT CATCH IT FOR
            // US, which is worth stating because it looks as though they would.
            // The rule's guard tests CMAKE_INSTALL_CONFIG_NAME -- the config passed
            // to `cmake --install` -- while the FILE PATH inside it is whatever
            // that tree's output directory is. So `--install --config Release` on a
            // RelWithDebInfo tree fires the Release rule and installs the
            // RelWithDebInfo binary under the name Player.exe, quietly. Measured on
            // a scratch tree, not assumed. This refusal is the only thing standing
            // between that and a shipped game, which is why it is an error.
            ...
            if (haveExe) {
                ...
                if (leaf == R.config) {
                    ...
                    // one.
                    //
                    // AND IT IS AN ERROR BECAUSE OF WHAT THE ANSWER IS USED FOR,
                    // not because the pipeline dislikes not knowing. profileExeDir
                    // is one of the paths the output directory is checked against
                    // below; a made-up value is a containment guard that passes
                    // when it should not, and the bundle it lets through is one
                    // written inside the build tree, which the staging step mirrors
                    // and deletes on the next build. A bundle that disappears is
                    // broken output.
                    Push(pf.errors,
            ...
            // ---- WILL THIS PROFILE'S BUNDLE HAVE A PLAYER IN IT ------------
            // A REFUSAL BECAUSE THE OUTPUT WOULD BE BROKEN, which is the bar
            // every error in this function has to clear: a bundle with content, a
            // manifest and no executable is not a game, and it fails at the moment
            // somebody double-clicks nothing.
            if (haveBinary && IsDir(R.binaryDir)) {
                const InstallRuleAnswer answer =
                    ProfileHasInstallRule(R.binaryDir, R.exeName, R.config);
                if (answer == InstallRuleAnswer::Absent) {
                    Push(pf.errors,
                         "this build tree installs no " + R.exeName + " for the " +
                             R.config + " configuration, so the " +
                             std::string(BuildProfileDisplayName(settings.profile)) +
                             " bundle would contain the whole game except the "
                             "program that runs it. Player/CMakeLists.txt installs "
                             "PlayerShipping for Release and PlayerDebug for Debug "
                             "and RelWithDebInfo; a tree generated before those "
                             "rules existed says this until CMake re-configures it, "
                             "which building anything in the tree does.");
                }
                else if (answer == InstallRuleAnswer::Unknown) {
                    // NOT A REFUSAL. "I could not read the answer" is not a defect
                    // in the output, and the check that CAN answer it -- validate,
                    // over the assembled bundle -- runs either way.
                    Push(pf.warnings,
                         "whether a " + R.exeName + " reaches the bundle could not "
                         "be worked out from the install scripts in '" +
                             R.binaryDir.generic_string() +
                             "'. The build still checks the assembled bundle for "
                             "it; only the advance answer is missing.");
                }
            }
            ...
            // ---- notes worth having ---------------------------------------
            // THE LINE BETWEEN THESE AND THE ERRORS ABOVE, stated once because it
            // is the rule the whole function is held to: an ERROR means the thing
            // this build would produce is BROKEN -- no executable, a scene that
            // will not load, a bundle written somewhere the next build deletes. A
            // NOTE means the build is unusual, and unusual is the author's to
            // choose.
            //
            // ONE SCENE IS A GAME. This used to be phrased as an instruction --
            // "add the rest of the game's scenes" -- which read as a demand and,
            // sitting under the panel's refusal heading, as a second reason the
            // build was refused. Plenty of shipped titles are exactly one scene,
            // and this project's own front end plus training mode is very nearly
            // that. What is worth saying is the consequence, not the correction:
            // the prune ships what is listed and nothing else, so a scene the
            // author expected to be in the game and did not list is simply absent.
            if (settings.scenes.size() == 1) {
                Push(pf.warnings,
                     "this build contains one scene ('" + settings.scenes.front() +
                         "'), which is a complete game -- nothing here needs more. "
                         "Worth a look only because the bundle carries exactly what "
                         "is in this list: any other scene the game reaches is "
                         "pruned out of it.");
            }
    ...
    struct BuildJob::Impl {
        ...
        //     Player/CMakeLists.txt:145-163 layers the editor-authored content with
    ...
    bool BuildJob::Impl::Assemble() {
        ...
        // CMAKE_INSTALL_CONFIG_NAME, which Player/CMakeLists.txt:145-163 uses to
        ...
        // Player/CMakeLists.txt:158-161 emits it when the configuration's staged
        ...
        for (const fs::path& f : candidates) {
            ...
            if (rel == kBuildSettingsRel) {
                // build.json: editor metadata the shipped game never reads. The
                // install(CODE) block now excludes it by name, so this normally
                // removes nothing -- it is the backstop for a third route into the
                // bundle, exactly like the sidecar case below, and like that one it
                // ANNOUNCES when it fires so it can never quietly become the reason
                // this bundle differs from `cpack`'s.
                reason = "editor build settings";
            }
            else if (ExtIs(f, ".import")) {
                // BOTH INSTALL RULES ALREADY EXCLUDE THESE (Player/CMakeLists.txt
                // :117, :151), so this normally removes nothing at all. It is kept as
    ...
    bool BuildJob::Impl::Validate() {
        ...
        //    install-rule refusal, and it is what lets that refusal answer Unknown
        //    instead of guessing: whatever the generated install script said or
        //    could not say, a bundle with no executable is not a build.
        if (!IsFile(R.outputDir / R.exeName)) {
            failures.push_back(
                "the bundle has no " + R.exeName + " in it, so there is nothing to "
                "run. The rule that should have put it there is " + R.target +
                "'s in Player/CMakeLists.txt, which installs only for the " +
