...
| P4-3 | Job system (thread pool) + async asset loading | **Phase 1 done 2026-07-18** (revived after the asset-architecture review concluded in-process jobs beat a separate asset app): `JobSystem` in Engine/src/core — fixed worker pool (hw/3 cap 4), mutex+condvar job queue, completions drained ONLY on the main thread via pumpCompletions(2ms/frame in RunLoop, GL context current), exception containment (throwing job still fires its completion), RunLoop drains on exit (review-caught: phase 2 quit-during-load would UAF Run()-locals otherwise), unpumped completions never run (contract asserted). HARD CONTRACT: jobs never touch GL/entt/ImGui; closures own their state. 10 headless tests (13th suite) incl. concurrent submit+pump stress; review made 2 tests falsifiable that weren't. **Next: phase 2 = split Model into DecodeModel (worker, Assimp/stb/LOD-index) + FinalizeGL (main); phase 3 = AssetManager::RequestModel handles + async AssetIndex scan; phase 4 = headless AssetCooker.exe when P4-2 is scheduled. Never: separate interactive asset app (ImGui docking/drag-drop/GL are same-process — design review 2026-07-18)** |
