...
| P1-5 | Decompose the `Renderer` God object: Application owns loop/window/input; Renderer renders — **and reimplement CSM shadows as part of this** | M–L | **User-reported (2026-07-10): shadows pop in/out and shimmer while the camera moves.** Suspects, in likelihood order: (a) round-robin cascade budget (1/frame) publishes fresh `lightVP` matrices every frame while depth textures lag behind → shader compares against stale maps during movement; (b) caster culling by camera-slice Z (`Scene.cpp` `RenderShadowsCombined`) drops casters that are outside the slice but still cast into it → popping; (c) update-policy epsilons (0.05 m / 0.5°) create hysteresis at thresholds; texel snapping may be defeated by per-frame FOV/aspect-driven extent changes. Plan: rebuild the CSM pass on the decomposed renderer — update all cascade matrices and depth maps atomically in the same frame, cull casters against the light-space AABB of the *receiver* slice (not the camera slice), and validate stability with a scripted camera-path test before/after. |
