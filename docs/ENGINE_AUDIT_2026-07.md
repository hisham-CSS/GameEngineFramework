...
| P4-3 | Job system (thread pool) + async asset loading | 6c/12t currently idle. **Deferred (user, 2026-07-13): still wanted eventually — primarily for async asset loading — but a later feature now that measurement showed the render path is GPU-bound, not CPU-bound (see P4-9)** |
