...
| S8 | Every field added to `GameState` is added to the reflection table in the same commit | compile-time + test | `cse/kernel/StateReflection.h`: the tables over `Event`, `Fighter` and `GameState` must cover every byte contiguously or a `static_assert` fails in any translation unit that includes it; `StateReflection.TheTableNamesEveryByteOfGameState` walks every byte to a field |
...
| N4 | Producer-side sticky state — the between-tick "pressed since last tick" accumulator — is never read during re-simulation and never networked | structural + test | `PressAccumulator` is spent into the input record BEFORE `LatchedInputSource` records it, so re-simulation reads only the latched log; `PressDelivery.TheDeliveredTapsAreInTheRecordAndReplayBitIdentically`. Over a session the same order holds: `UntitledFighterMode::FixedTick`'s live branch spends the accumulator into the offer BEFORE the session records it, and only on a frame the session will take (`ISession.h` rule 5, `SessionDriver::AcceptsInput`), so what the peer receives is the spent record and a tap made during a stall waits for the frame that carries it; `SessionDriver.AFrameWithNoAdvanceRunsZeroTicksAndDropsNone` |
| N5 | UI focus never suppresses gameplay input while a session is live | test + structural | `GateFrame` (`Engine/src/core/FrameGate.h`) never suppresses the pad while `Application::sessionLive()` is set and `RunLoop` applies what it returns; the mode raises the flag in `AttachSession` and lowers it on detach and Exit. `FrameGate.ALiveSessionOwnsTimeAndThePad` |

### The tick loop (T)

| Id | Rule | Enforced by | Where |
|---|---|---|---|
| T1 | The session decides how many ticks run. Zero ticks this frame is legal; **dropping a tick is not** | test | `SessionDriver` (`Games/UntitledFighter/Modes/src/SessionDriver.h`) runs exactly the session's Advance events: `SessionDriver.RunsExactlyTheTicksTheSessionAdvances`, `.AFrameWithNoAdvanceRunsZeroTicksAndDropsNone`; the mode with a session attached: `FightMode.AFrameTheSessionDoesNotAdvanceRunsNoTickAndIsNotAnError` |
| T2 | `FixedTimestep` never decides the simulation's tick count — it caps at 8 steps and then zeroes the accumulator, discarding the backlog | test + structural | the fixed step PUMPS and never counts: `FixedTick`'s live branch calls `SessionDriver::Frame` once per step and takes the tick count from its return, so a dropped backlog is a late pump, not a lost tick; `SessionDriver.RunsExactlyTheTicksTheSessionAdvances` measures more ticks than pumps under the stress session's rollbacks. The reason is written at the top of `GameState.h` |
| T3 | While a session is live, pause, time scale and scene swap are inert | test + structural | pause and time scale: `GateFrame` hands the raw frame dt through while a session is live (`FrameGate.ALiveSessionOwnsTimeAndThePad`); the mode's own pause, frame step, slow motion, reset, character swap, stage position, Demonstrate and hot reload are inert while attached (`FightMode.PauseStepAndSlowMotionAreInertWhileASessionIsLive`); scene swap: the session and its driver are members of the mode, not of the scene, and `RunLoop` skips one pump on the swap frame while the session decides the ticks |
| T4 | Every tick can produce a checksum; a session exchanges one periodically and stops the match on mismatch | test | `Checksum()` and `KernelRollback.ChecksumDetectsASingleBitOfDivergence`; the exchange between two kernels and the report: `Session.ADivergentPeerIsReportedAndNamed` ([ADR-021](adr/ADR-021-transport.md)); the shipped mode polls the report every live step and stops the match on it, naming the field: `FightMode.ADesyncReportEndsTheMatchAndNamesTheFieldOnTheHud` |
...
| T6 | A desync is reported and the match stops. It is never silently corrected | test | `Session.ADivergentPeerIsReportedAndNamed` names the frame and both checksums; `Desync.TwoPeersAbortAndTheArtifactNamesTheTickAndField` ends the sessions, swaps the two states and names the first divergent field (`cse::game::FirstDivergence` over `StateReflection.h`); the shipped mode does the same as its last act — `FightMode.ADesyncReportEndsTheMatchAndNamesTheFieldOnTheHud` sees both peers' ticks stop, the field on the HUD and in `desync_slot<N>.json`, and the match controls inert after it; `ISession::PollDesync` remembers it for the session's life; there is no correction path to disable |
...
| A4 | The handshake hashes the **loaded POD arrays**, never the source text — canonicalising text is where float-repr and key-order bugs live | test | `HashMatchData` (`Games/UntitledFighter/Game/include/cse/game/Replay.h`) is what `cse::net::HandshakeOffer` carries; `Handshake.HashesTheLoadedPodArraysNotTheText` re-spells the shipped file and gets the same hash, adds one frame and does not |
| A5 | A content mismatch is a lobby error naming the reason, never a gameplay bug | test | `Handshake::Compare` names the first disagreeing field with both values before any session exists: `Handshake.AContentMismatchIsALobbyErrorNamingTheHash`, `.TheFirstDisagreementIsTheReason`; `test_online_two_peers` sees it over UDP |
