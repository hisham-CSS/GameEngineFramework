...
| T6 | A desync is reported and the match stops. It is never silently corrected | test (reported, named, aborted in the harness) + review (the mode stops, ROADMAP M2.5) | `Session.ADivergentPeerIsReportedAndNamed` names the frame and both checksums; `Desync.TwoPeersAbortAndTheArtifactNamesTheTickAndField` ends the sessions, swaps the two states and names the first divergent field (`cse::game::FirstDivergence` over `StateReflection.h`); `ISession::PollDesync` remembers it for the session's life; there is no correction path to disable |
