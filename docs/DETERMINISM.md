...
| P4 | Pose is a pure function of `(moveId, moveFrame, posX, posY, facing, stance, the stun fields, tick)`. A return-to-idle tail is presentation only, is interrupted the tick the simulation acts, and can never delay a move, move a box or hold a fighter in place; a presentation-side blend out of a tail is **bounded** (default 4 frames) and subject to the same three nevers | **selector: test** · pose: not yet | the SELECTION half — which clip, which frame — is `cse::game::SelectPose`, pinned by `tests/test_pose_select.cpp` (`RestoreAndResimulateReproduceEveryPose`, `WhileAMoveRunsTheFrameIsTheMoveFrame`, `HitstopFreezesThePose`, `NeverTouchesTheChecksum`); the pose itself does not exist yet — the box overlay is all that draws a fighter. [ADR-011](adr/ADR-011-mechanics-are-fields.md) decision 6 is the rule; ROADMAP M3.2–M3.4d build the rest and own the remaining acceptance tests. Under [ADR-019](adr/ADR-019-placeholders-through-blender.md) D3 the placeholder holds NO presentation state at all (no tail, no blend), so the bound above is the recorded reversal, not the plan |
