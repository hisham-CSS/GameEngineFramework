...
2. **Pick the scene path.** Command line wins, then project settings: the scene is the **first argument that does not begin with `--`**, so `Player.exe path/to/scene.json` overrides `ProjectSettings::startupScene`, and a `--key` consumes the value after it — `Player.exe --slot 1 --port 47012 --peer 127.0.0.1:47011` boots the default scene and leaves every `--key value` pair on `Application::commandLine()` for a linked title's mode to read (`PlayerMain.cpp` names no key; the fighter's Versus lobby is the reader today — [fighting-core.md](fighting-core.md#the-modes-three-intents-one-presentation)). `project.json` is loaded either way, because the master volume comes from it too.
...
There is exactly one staging target, `runtime_assets`, defined in the root `CMakeLists.txt` and depended on by `Editor`, `PlayerDebug`, `PlayerShipping`, and `AssetCooker` (`Cooker/CMakeLists.txt`). Every executable that shares the output directory takes a dependency on that one target rather than copying for itself: concurrent copies into the same directory race under Ninja, so one writer is the rule. A fifth consumer added later belongs on the same `add_dependencies(... runtime_assets)` line, never on a copy step of its own.
