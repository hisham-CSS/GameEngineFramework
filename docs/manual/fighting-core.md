...
**And since ADR-015 (accepted 2026-09-01, option 3), every analysis answers PER OPENING.** `ProverOpening` (`ProverAdapter.h`) names the three ways a string can start — **neutral** (the grounded hit every verdict above describes), **counter**, and **air** — and `ProverResult::openings` carries a complete result for each; the top-level fields stay the *neutral* answer verbatim, so nothing that reads the single-verdict surface changed meaning. Which numbers differ: **counter is identical to neutral by construction** until M1.3(c) makes `counter_hit` authorable (the tests pin the identity); **air substitutes each move's authored `air_hitstun_ticks`** — a verdict about the *file's* air numbers, which the kernel does not yet simulate (M1.3(d)); the loss ledger says so, and the panel and `DescribeVerdict` print one line per opening under the banner. `AnalyseCharacter` runs the model once per opening — three runs cost well under a millisecond.
