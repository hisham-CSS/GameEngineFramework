...
A09–A20 are the schema-v3 assertions (invincibility, boxes, motion keys); their register is `x-load-assertions` in `Games/UntitledFighter/Assets/Characters/schema.v2.json`, which is the one place every rule id is listed. A21 and A22 are off by default: a character with no `engine.anim3d.model` checks neither, and the model itself is never opened by the loader — only its sidecar, read with nlohmann. The model path is authored relative to the character file, the way a glTF's own URIs are, resolved from `LoadOptions::fileDir` and contained against `LoadOptions::contentRoot` like every authored path, then stored root-relative for the presentation (`CharacterData.PresentationModelIsOptionalAndSandboxed`, `CharacterData.TheModelPathResolvesFromTheCharacterFilesDirectory`). Both assertions run on a variant's MERGED document too, so a variant that adds a move needs a clip of that move's duration or drops the model: `fighter_a/variants/jump_cancel.json` drops it and fights in 2D. An unknown key under `engine.anim3d` at either level is an ordinary load error naming the key (`CharacterData.AnAnim3dKeyOutsideTheContractIsALoadErrorNamingTheKey`).
...
> **What the transport proves, and where.** `Session.TwoPeersOverALoopbackTransportAgreeOnEveryChecksum`, `.LatencyForcesRollbacksAndBothPeersConverge` and `.LossIsSurvivedByTheSessionsOwnRedundancy` in `tests/test_session.cpp` run two online sessions in one process through a `LoopbackNetwork` and hold every confirmed frame to one checksum on both sides; `.ADivergentPeerIsReportedAndNamed` makes one kernel drift and sees the desync named with its frame and both checksums ([DETERMINISM.md](../DETERMINISM.md) T4, T6). `tests/two_peers.py` runs two `online_peer` processes over `UdpTransport` on 127.0.0.1 and requires the same checksum at the same frame (`test_online_two_peers`). GekkoNet's own asio adapter is not compiled in this tree, on purpose (`ThirdParty/CMakeLists.txt`); the bridge from `ITransport` to its adapter lives in `GekkoSession.cpp` and nowhere else. The connect handshake, the desync artifact and the tick count are the three paragraphs below.

**The connect handshake** (`Net/include/cse/net/Handshake.h`, ROADMAP M2.2) hashes the **loaded POD arrays** — `HashMatchData` over the built `MatchData`, not the canonicalized text — and puts it in a `HandshakeOffer` with the state and input sizes, the seed, the player count and the slot; each peer sends its offer on every pump over the session's own transport and the two are compared in one fixed order, so the first disagreement is the lobby error, naming the field and both values, never "desync at tick 3" ([DETERMINISM.md](../DETERMINISM.md) A4–A5). The `Handshake` is itself an `ITransport` wrapping the real one: create the session on it once it reports `Agreed`, keep pumping it for its grace resends, and it peels the peer's late offers off while handing the session everything else. The reason it is the `MatchData` and not the state: a `GameState` alone no longer describes a match. It is meaningless without the data it was simulated against, and proving both peers hold the same one is the handshake's job. `tests/test_handshake.cpp` holds all of it; `tests/two_peers.py` sees a mismatch refused by name over UDP before it agrees a real match.
