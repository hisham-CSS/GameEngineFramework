...
The default is a **pure sky** — sky only, with no ground, terrain, or buildings
baked in. That is deliberate: a location-specific HDRi imposes someone else's
horizon on every scene you make, which reads as wrong the moment your own
geometry does not line up with it. It is CC0 from Poly Haven; see
`Exported/Env/CREDITS.md`.

A second example ships at `Exported/Env/studio.hdr` — type that path into the HDRi
field to try it. It is generated by `tools/make_example_env.py` rather than
downloaded, so it is reproducible and small (105 KB). It deliberately contains
three bright rectangular softboxes: a blurry gradient looks fine even when the
specular prefilter chain is wrong, whereas rectangular highlights that smear
smoothly from sharp to soft as roughness rises are proof it is correct.

For real-world environments, [Poly Haven](https://polyhaven.com/hdris) publishes
CC0 HDRis that drop straight into `Exported/Env/`.
